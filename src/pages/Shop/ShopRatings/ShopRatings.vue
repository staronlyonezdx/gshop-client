...
<script>
  ...
  export default {
    data() {
      return {
        selectRateType: 0,//0表示满意,1表示不满意,2表示全部
        onlyContent: true//true表示只看有评论内容的,false表示看没有评论内容的
      }
    },
    mounted() {
    ...
    computed: {
      ...
      FilterRatings() {
        ...
        return ratings.filter((rating) => {
          ...
          selectRateType === 2 ||　rating.rateType === selectRateType
          */
          /*
          条件2
          ...
          */
          return (selectRateType === 2 || rating.rateType === selectRateType) && (!onlyContent || rating.text.length > 0)
        })
      }
    },
    methods: {
